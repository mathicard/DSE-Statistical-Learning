...
As mentioned before, PCA is a powerful tool for data visualization. In this case, there is a tradeoff between visualization and reliability of the model. On one hand, the fourth component has a not significant explanatory importance since the eigenvalue is not above 1 and it moves the cumulative variance explained from 84% to 90%. On the other hand, it is slightly below the threshold and could be neglected without a significant loss. In order to have a higher explained variance one would select a four-dimensional model, while a lower-dimensionality fashion for plotting requires to have at most three components.


#### Three components analysis

Since in this analysis PCA is used for description and visualization, we focus more on the three components analysis. The three component analysis is the most useful one to read loadings and thus understand how variables are explained by the principal components. First of all, we select the corresponding eigenvectors. 
