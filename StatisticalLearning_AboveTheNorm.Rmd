...
Our dataset, which is composed by 51 observations (the 51 countries of the United States of America) and 18 explanatory variables, does not need any other manipulation since we do not have any missing values. The only things we have done are: the transormation of the dependent variable (the Air Quality) with the logarithmic scale, in order to get a "more" normal distribution as explained in the previous section, and the normalization of all our variables, which will give us the rescaling with the same unit of measurements. 
Once our dataset was ready, we had an attempt with the simplest way we know to construct a model: the Ordinary least Square. In the full model the most significative variables semm to be the number of factories (with positive sign, as expected) and precipitations (with negative sign, as expected). 
...
Now in our linear model made up by the 4 variables selected, the only one which becomes no more significative is the rural population indicator, but te signs are all as expected.
...
At least our new model created with the forward search has a lower Mean Squared Error on the prediction made on the test set. Before it was 0.1512593, now it is 0.1221634.
